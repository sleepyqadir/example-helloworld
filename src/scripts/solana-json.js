...
const solanaJSON = {
    ...
    createUser: async (secret) => {
        let user;
        if (secret) {
            const secretKey = typeof secret === 'string' ? Uint8Array.from(JSON.parse(secret)) : secret;
            user = solanaWeb3.Keypair.fromSecretKey(secretKey);
            return user
        }

        user = new solanaWeb3.Keypair();
        const stringSecretKey = JSON.stringify(Array.from(user?.secretKey));
        console.log({
            publicKey: user.publicKey,
            secretKey: user.secretKey,
            stringPublicKey: user.publicKey.toString(),
            stringSecretKey,
        });
        console.log(`New solana account created: ${user.publicKey}`, user.secretKey);
        return user;
    },
